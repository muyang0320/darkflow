...
def camera(self):
    ...
    camera = cv2.VideoCapture(file)# 获取摄像头对象(如果keyword为camera，那么file==0，一般就是笔记本内建摄像头)
    ...
    buffer_inp = list() # 存放VideoCapture read的frame
    buffer_pre = list() # 存放预处理过的frame
    
    elapsed = int() # elapsed == 0
    start = timer()
    self.say('Press [ESC] to quit demo')
    # Loop through frames
    while camera.isOpened(): # camera = cv2.VideoCapture(file)
        elapsed += 1
        _, frame = camera.read() # 获得一帧
        if frame is None:
            print ('\nEnd of Video')
            break
        preprocessed = self.framework.preprocess(frame)
        ...
        if elapsed % self.FLAGS.queue == 0: #也就是说当elapsed == self.FLAGS.queue
